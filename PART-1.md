...
### Tasks
  ...
  - For each task fineshed, do a commit. Use the task title if want :wink:
...
Pass the `films` array stored in `TMDB.js` to each of your new components as props.
...
Create a new component for each film row, called `FilmRow.js`. Have your `map()` (from the step above) create an array of `FilmRow` components instead. Don't forget to pass the individual film prop to the component when creating them!. at this point `FilmRow` should be rendering a `<h1>` contenning the filme title's. the page should look like the same.
...
**create a pull request to submit your work!!!** :wink: